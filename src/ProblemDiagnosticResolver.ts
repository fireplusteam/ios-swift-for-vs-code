...
export class ProblemDiagnosticResolver implements HandleProblemDiagnosticResolver {
    ...
    public handleDiagnostics(
        ...
        // errors generated by swift macros are not coming from source kit, that's why we need to treat them separately
        const diagnostics =
            this.diagnosticBuildCollection
                .get(uri)
                ?.filter(e => this.isDiagnosticFromSwiftMacroError(e)) || [];
        this.diagnosticBuildCollection.set(uri, [...diagnostics, ...newDiagnostics]);
    }
